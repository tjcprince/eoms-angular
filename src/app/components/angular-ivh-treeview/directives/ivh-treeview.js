...
/*eslint-disable no-unused-vars*/
  angular.module('ivh.treeview').directive('ivhTreeview', ivhTreeview);
  /** @ngInject */
  function ivhTreeview(ivhTreeviewMgr) {
    'use strict';
    return {
      restrict: 'A',
      transclude: true,
      ...
      controller: ['$scope', '$element', '$attrs', '$transclude', 'ivhTreeviewOptions', 'filterFilter', function($scope, $element, $attrs, $transclude, ivhTreeviewOptions, filterFilter) {
        var ng = angular,
          trvw = this;
        //选中的节点--tjc
        trvw.selectedNode = function(node, element) {
            $scope.selectedNode = node;
            if (typeof($scope.selectedEl) != "undefined") {
              //取消上次选中的样式
              $scope.selectedEl.removeClass("ivh-treeview-selected");
            }
        ...
        trvw.isVisible = function(node) {
          ...
          // separately
          if (angular.isObject(filter) || angular.isFunction(filter)){
      ...
      link: function(scope, element, attrs) {
        ...
        //暴露控制树的方法--tjc
        if (scope.eomsIvhTreeControl != null) {
          if (angular.isObject(scope.eomsIvhTreeControl)) {
            var tree = scope.eomsIvhTreeControl;
            //选中的节点--tjc
            tree.selectedNode = function() {
